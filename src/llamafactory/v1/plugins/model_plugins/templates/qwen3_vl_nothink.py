...
@RenderingPlugin("qwen3_vl_nothink").register("render_messages")
def render_qwen3_vl_nothink_messages(
    processor: Processor,
    ...
    """Render messages in the Qwen3 VL nothink template format."""
    ...
    for turn_idx, message in enumerate(messages):
        if message["role"] == "user" or (message["role"] == "system" and turn_idx != 0):
            ...
            for content in message["content"]:
                if content["type"] == "text":
                    temp_str += content["value"]
                elif content["type"] in ["image", "image_url", "video"]:
                    ...
                    raise ValueError("Audio is not supported in qwen3_vl_nothink.")
                else:
                    raise ValueError(f"Unsupported content type: {content['type']}")
...
def process_multimodal_content(
    ...
    if content["type"] in ["image", "image_url"]:
        ...
        image_seqlen = (
            int(image_grid_thw[0].prod().item() // image_merge_length) if expand_mm_tokens else 1
        )
    ...
    if content["type"] == "video":
        ...
        for frame_index in range(num_frames):
            video_seqlen = (
                int(video_grid_thw[0][1:].prod().item() // video_merge_length)
                if expand_mm_tokens
                else 1
            )
...
if __name__ == "__main__":
    ...
    sample_base = {
        "messages": [
            ...
            {
            "role": 'user',
            "content": [{"type": "text", "value": "Can you tell me a joke?"}, {"type": "video", "value": "/home/j30074199/tasks/llamafactory/template/LlamaFactory/data/mllm_demo_data/1.mp4"}],
            },
        ],
    ...
    sample_reasoning = {
        "messages": [
            ...
            {
            "role": 'user',
            "content": [{"type": "image", "value": "/home/j30074199/tasks/llamafactory/template/demo.jpeg"}, {"type": "text", "value": "What is the capital of France?"}],
            },
            {
            "role": 'assistant',
    ...
    rendered_output_base = RenderingPlugin("qwen3_vl_nothink").render_messages(processor, sample_base["messages"])
    # rendered_output_reasoning = RenderingPlugin("qwen3_vl_nothink").render_messages(processor, sample_reasoning["messages"])
    # rendered_output_tool_usage = RenderingPlugin("qwen3_vl_nothink").render_messages(processor, sample_tool_usage["messages"], tools=tools)
