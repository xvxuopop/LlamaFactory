    ...
    for idx, message in enumerate(reversed(messages)):
        ...
        for content in message["content"]:
            if not (message["role"] == "user" and 
                content["type"] == "text" and 
                not (content["value"].startswith("<tool_response>") and content.endswith("</tool_response>"))):
                break
...
if __name__ == "__main__":
    ...
    rendered_output_base = RenderingPlugin("qwen3").render_messages(processor, sample_base["messages"])
    rendered_output_reasoning = RenderingPlugin("qwen3").render_messages(processor, sample_reasoning["messages"])
    rendered_output_tool_usage = RenderingPlugin("qwen3").render_messages(processor, sample_tool_usage["messages"], tools=tools)
    ...
    generated_text = "<think>\nThe user is asking for the capital of France. This is a factual question. I know this information.\n</think>\n\nThe capital of France is Paris."
    parse_text = RenderingPlugin("qwen3").parse_message(generated_text)
    print(parse_text)